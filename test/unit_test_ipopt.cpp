...
class GenericLinearConstraint : public bopt::LinearConstraint {
   ...
   protected:
    ...
    void setJacobianSparsityPatterns(EvaluatorData &data) const override {
        ...
        data.Jx_s.setFromTriplets(triplets.begin(), triplets.end());
    }

    void evalSparseJacobiansImpl(const Eigen::Ref<const VectorXd> &x,
                                 EvaluatorData &data, bool compute_x,
                                 bool compute_p) const override {
        VLOG(10) << "evalSparseCoefficientMatrixImpl";
        data.Jx_s.valuePtr()[0] = 1;
        data.Jx_s.valuePtr()[1] = 1;
        data.Jx_s.valuePtr()[2] = 1;
    }
