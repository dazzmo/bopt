...
class GenericQuadraticCost : public bopt::QuadraticCost {
   ...
   protected:
    void evalImpl(const Eigen::Ref<const VectorXd> &x,
                  bopt::CostData &data) override {
        data.f = x.squaredNorm();
    }

    void evalCoefficientMatrixImpl(bopt::QuadraticCostData &data) override {
        data.A.setIdentity();
    }

    void evalCoefficientVectorImpl(bopt::QuadraticCostData &data) override {
        data.b.setZero();
    }
};
...
TEST(Program, SimpleProgram) {
    ...
    auto data = bopt::CostData(*c);

    VLOG(10) << data.Hxx_s.rows();
    VLOG(10) << data.Hxx_s.cols();
    VLOG(10) << data.Hxx_s.size();
    // auto g0 = std::make_shared<GenericLinearConstraint>();

    // bopt::EvaluatorBaseDataTpl<double> data(*c);

    // VLOG(10) << data.Jx;

    // Eigen::MatrixXd m = Eigen::MatrixXd::Identity(1, 2);
