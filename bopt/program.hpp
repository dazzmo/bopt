...
void getJacobianStructure(MatrixContainer &J,
  ...
  for (auto &b : bindings) {
    ...
    for (index_type col = 0; col < info.out_m; ++col) {
      index_type start = ccs_traits<evaluator_out_info_t>::indptr(info)[col];
      index_type end = ccs_traits<evaluator_out_info_t>::indptr(info)[col + 1];

      for (index_type row = start; row < end; ++row) {
        // Get index of entry
        index_type idx = ccs_traits<evaluator_out_info_t>::indices(info)[row];
        // Add entry to full Jacobian
        inserter(J, b.get()->input_indices[0][idx], cnt + col, value_type(0));
      }
    }
...
class mathematical_program {
 public:
   ...
   * values.
   */
  const std::vector<value_type> &variable_initial_values() const { return x0_; }
  ...
  void add_variables(const std::vector<variable> &variables,
                     const std::vector<value_type> &v0 = {},
                     const std::vector<value_type> &bl = {},
                     const std::vector<value_type> &bu = {}) {
  ...
  void add_parameters(const std::vector<variable> &parameters,
                      const std::vector<value_type> &val = {}) {
    for (auto it = parameters.begin(); it != parameters.end(); ++it) {
      index_type i = std::distance(parameters.begin(), it);
      if (val.empty()) {
        add_parameter(*it, value_type(0));
      } else {
        add_parameter(*it, val[i]);
      }
    }
  }
  ...
  void add_linear_cost(const typename linear_cost<value_type>::shared_ptr &cost,
                       const input_variable_vector &x,
                       const input_parameter_vector &p) {
    // Create binding
    input_index_vector indices = create_input_index_vector(x, p);
  ...
  std::vector<binding<linear_cost_t>> &linear_costs() { return costs_linear_; }
  ...
  std::vector<binding<bounding_box_constraint_t>> &bounding_box_constraints() {
