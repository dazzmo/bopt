...
bool IpoptSolverInstance::get_nlp_info(Index& n, Index& m, Index& nnz_jac_g,
  ...
  // todo - if too large, split it up and estimate in smaller sets
  Eigen::MatrixXd J, H;
...
bool IpoptSolverInstance::eval_f(Index n, const Number* x, bool new_x,
                                 Number& obj_value) {
  bopt::Profiler profiler("IpoptSolverInstance::eval_f");
  VLOG(10) << "eval_f()";
...
bool IpoptSolverInstance::eval_grad_f(Index n, const Number* x, bool new_x,
                                      Number* grad_f) {
  bopt::Profiler profiler("IpoptSolverInstance::eval_grad_f");
  VLOG(10) << "eval_grad_f()";
...
bool IpoptSolverInstance::eval_g(Index n, const Number* x, bool new_x, Index m,
                                 Number* g) {
  bopt::Profiler profiler("IpoptSolverInstance::eval_g");
  VLOG(10) << "eval_g()";
  if (new_x) mapVector(cache_.primal, x, n);
...
bool IpoptSolverInstance::eval_jac_g(Index n, const Number* x, bool new_x,
  ...
  bopt::Profiler profiler("IpoptSolverInstance::eval_jac_g");
  VLOG(10) << "eval_jac_g()";
  if (values == NULL) {
...
bool IpoptSolverInstance::eval_h(Index n, const Number* x, bool new_x,
                                 ...
                                 Number* values) {
  bopt::Profiler profiler("IpoptSolverInstance::eval_h");
  VLOG(10) << "eval_h()";
  if (values == NULL) {
    // Return the sparsity of the constraint Jacobian
    int cnt = 0;
